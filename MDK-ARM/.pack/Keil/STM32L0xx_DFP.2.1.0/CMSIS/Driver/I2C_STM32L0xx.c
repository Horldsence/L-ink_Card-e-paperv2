 ...
 * Project:      I2C Driver for STMicroelectronics STM32L0xx
 * --------------------------------------------------------------------------
...
/*! \page stm32l0_i2c CMSIS-Driver I2C Setup
...
----------
  1. Configure peripheral
     - Peripherals \b I2C1 enable <b>I2C</b>

  2. Configure pins PB6 and PB7 as I2C1 peripheral alternative pins
     - Click in chip diagram on pin \b PB6 and select \b I2C1_SCL
     - Click in chip diagram on pin \b PB7 and select \b I2C1_SDA
...
-----------------
  1. Under Connectivity open \b I2C1 Configuration:
     ...
     - <b>NVIC Settings</b>: enable interrupts
          ...
          I2C1 event global interrupt / I2C1 wake-..| \b ON   | 0

     - \e optional <b>DMA Settings</b>: setup DMA transfers for Rx and Tx (DMA is optional)\n
       \b Add - Select \b I2C1_RX: Stream=DMA1 Stream 3, Direction=Peripheral to Memory, Priority=Low\n
          DMA Request Settings         | Label             | Peripheral | Memory
          :----------------------------|:------------------|:-----------|:-------------
          Mode: Normal                 | Increment Address | OFF        |\b ON
          .                            | Data Width        |\b Byte     | Byte
       \b Add - Select \b I2C1_TX: Stream=DMA1 Stream 2, Direction=Memory to Peripheral, Priority=Low\n
          DMA Request Settings         | Label             | Peripheral | Memory
          :----------------------------|:------------------|:-----------|:-------------
          Mode: Normal                 | Increment Address | OFF        |\b ON
          .                            | Data Width        |\b Byte     | Byte
     ...
     Click \b OK to close the I2C2 Configuration dialog
*/

/*! \cond */

#include "I2C_STM32L0xx.h"
...
static int32_t I2C_Control (uint32_t control, uint32_t arg, I2C_RESOURCES *i2c) {
  ...
  switch (control) {
    ...
    case ARM_I2C_BUS_SPEED:
      if (i2c->periph_clk != 0U) {      // For I2C1 and I2C3
        fpclk = HAL_RCCEx_GetPeriphCLKFreq(i2c->periph_clk);
      } else {                          // For I2C2
